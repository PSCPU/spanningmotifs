###########################################################################################################################
#                                          Identification of spanning motifs - 4                                          #
###########################################################################################################################
...
# Read amin acid-mediated spanning motif generated by script '3_Remove_fasle_spanning_motifs'
file = open('AA_spanningmotifs.txt', 'r')
out = []
i = -1
...
for l in range(len(my_dict['val'])):   
    for lines in my_dict['val'][l]:
        ...
        if '[' in lines:
            inter = lines.split('---')
            for i in inter:
                if i.strip()[:9] not in aa: aa.append(i.strip()[:9])
                if i.strip()[14:23] not in nt: nt.append(i.strip()[14:23])
